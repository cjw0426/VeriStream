...
CLIP-TopK 选择。Early VeriStream 是已经完成的早期工具化框架，用于判断主动历史检索是否有效；最新
版 VeriStream 的全量实验尚未跑完，表中暂以 TODO 标记，完成后统一更新：
...
Early VeriStream 相对 Recent4 已在 Backward 上取得 **4.15 个百分点的明显提升**，同时仅损失 2.07 个
百分点的 Realtime，使 BR 平均提高 1.04 个百分点。这说明主动检索历史记忆的方向本身有效，而不是只
增加系统复杂度。这里的“明显提升”描述数值差异；在逐样本 paired bootstrap 完成前，不把它表述为统计
显著性。

另一方面，Early VeriStream 仍比 Uniform16 Action-fact 低 4.01 / 0.25 / 2.14 个百分点，也比
CLIP-TopK16 Action-fact 低 3.86 个 Backward 和 1.41 个 BR 平均百分点。Trace 显示，收益被时间范围
误判、重复 Search、Inspect/Compare 不稳定和过早 Finish 部分抵消。因此最新版 VeriStream 并未推翻
“工具检索历史”的方向，而是用受验证 Controller、可拒绝检索、L1→L0 核验和时间证据图修复这些执行
漏洞。其全量结果仍在运行，TODO 行不参与当前排名或结论。
...
$\bar H_t=\operatorname{Uniform32}(H_t)$，并输出 $\pi_T(y_t\mid\bar H_t,q)$。Student 由原始模型
$\operatorname{MLLM}_0$ 和可学习 memory module $M_\theta$ 组成，只接收稀疏观察 $o_t\subset H_t$ 与
当前 compact memory：
...
停止。Early VeriStream 已验证主动历史检索可以提高 Backward；最新版方法进一步针对检索、核验与控制
漏洞进行结构化修复。该设计满足 training-free、多模态工具、持久 Memory 和感知/推理解耦要求；相对
同基座 Recent4 与 Action-fact 对照的最终收益，将在自主 Controller 全量实验完成后更新。进一步地，
结构化轨迹 SFT 和 on-policy memory distillation 分别提供了“学习工具推理范式”和“学习 memory policy”
两条互补扩展路径，但二者均超出本文 Training-free 设定。
